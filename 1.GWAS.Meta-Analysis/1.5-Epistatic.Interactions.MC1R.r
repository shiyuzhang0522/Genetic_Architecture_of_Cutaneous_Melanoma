###############################################################################
# MC1R missense variants × circulating KIT
#
# Additive and interaction models with covariate adjustment,
# plus descriptive 4-group genotype comparison.
#
# Author: Shelley
# Date:   2025-10-31
###############################################################################
...
## -----------------------------
## 4) Define genotype columns
## -----------------------------
stopifnot("kit_irnt" %in% names(merged_dt))

v1_raw <- "DRAGEN:chr16:89919510:C:A"
v2_raw <- "DRAGEN:chr16:89919709:C:T"
...
## -----------------------------
## 8) Descriptive 4-group analysis
## -----------------------------
dt <- copy(ana_dt)
...
p_lab    <- paste0("ANOVA P = ", formatC(p_global, format = "e", digits = 2))

sum_dt <- dt_plot[, .(
  mean_kit = mean(kit_irnt),
  sd_kit   = sd(kit_irnt),
  n        = .N
), by = geno4]
...
p <- ggplot(sum_dt, aes(x = geno4, y = mean_kit)) +
  ...
  geom_errorbar(aes(ymin = ci_l, ymax = ci_u, color = geno4), width = 0.16, linewidth = 0.75) +
  geom_point(aes(color = geno4), size = 1.7) +
  geom_point(data = expected_df, aes(x = geno4, y = expected),
             shape = 4, size = 2.0, color = "grey35") +
  scale_color_manual(values = col_map) +
  labs(x = NULL, y = "KIT (IRNT)", subtitle = p_lab) +
  theme_classic(base_size = 10)

ggsave("MC1R_KIT_interaction_plot.github.pdf",
       p, width = 3.8, height = 3.0, device = cairo_pdf)

fwrite(sum_dt, "MC1R_KIT_genotype_group_summary.tsv", sep = "\t")
